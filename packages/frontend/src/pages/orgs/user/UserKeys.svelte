<script lang="ts">
    ...
    const onKeyDelete = (id: string) => {
        $orgCtx.myKeys = $orgCtx.myKeys.filter((e) => e.id !== id);
    };
</script>
...
{#if $orgCtx.myKeys.length === 0}
    <CardGrid class="mt-6">
        <InductionStepCard title="Keys secure your responses">
            Every response to your form will be encrypted with your key. This
            means nobody outside your organisation can see it — not even us!
        </InductionStepCard>
        <InductionStepCard title="Stored in your browser">
            That's right — your plain key is super secret so we keep it in your
            browser and never send it to our server (unless you make a backup).
        </InductionStepCard>
        <InductionStepCard title="Backups">
            Losing your form responses would be really sad, so you can back up
            your keys to our server, with a secret recovery phrase to keep them
            secure.
        </InductionStepCard>
    </CardGrid>
{:else}
    ...
    <Alert color="gray" border class="mt-4">
        ...
        <p>
            ...
            responses can be decrypted with these keys, but new responses won't
