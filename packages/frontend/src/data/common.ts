...
import { apiWithAuth, apiWithCaptcha } from "./auth";
import { AxiosError } from "axios";
import { is_api_error_js } from "@palform/palform-client-common";
...
export const APIs = {
    ...
    authWithCaptcha: apiWithCaptcha(AuthenticationApi, baseAPIConfig),
    authWithToken: () => apiWithAuth(AuthenticationApi, baseAPIConfig),
    secondFactors: () => apiWithAuth(Class2FAMethodsApi, baseAPIConfig),
    captcha: new CaptchaApi(new Configuration(baseAPIConfig)),
    ...
    fill: (fillAccessToken: string, captchaResponse?: string) => {
        ...
        return {
            ...
            submissionAssets: new SubmissionAssetApi(config),
        };
    },
    ...
    feedback: new FeedbackApi(new Configuration(baseAPIConfig)),
};
