...
import type { BaseAPI } from "@palform/palform-typescript-openapi/src/base";
import {
    Configuration,
    type ConfigurationParameters,
    type NewAPIAuthToken,
    type SecondFactorRequiredSecondFactorRequired,
...
export function getOrgSubdomain() {
    ...
    if (subdomain === "dash" || subdomain.startsWith("dash-")) return null;
    return subdomain;
}
...
export async function signInWithEmailAndPassword(
    ...
    APIs.auth;
    const response = await APIs.authWithCaptcha(captcha).authSignIn({
        email,
        password,
...
export function apiWithCaptcha<T extends BaseAPI>(
    api: { new (configuration: Configuration): T },
    configuration: ConfigurationParameters
) {
    return (captchaResponse: string) => {
        return new api(
            new Configuration({
                apiKey: (name) => {
                    if (name === "X-Captcha-Response") {
                        return captchaResponse;
                    } else {
                        throw new Error(`Unknown API key type ${name}`);
                    }
                },
                ...configuration,
            })
