...
export function moveQuestion(
    ...
    ctx.update((ctx) => {
        const i = ctx.questions[groupId].findIndex((e) => e.id === question.id);
        if (
            (direction === "up" && i === 0) ||
            (direction === "down" && i === ctx.questions[groupId].length - 1)
        )
            return ctx;

        ctx.questions[groupId].splice(i, 1);
        ctx.questions[groupId].splice(
